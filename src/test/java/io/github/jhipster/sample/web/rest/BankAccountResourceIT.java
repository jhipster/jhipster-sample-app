...
public class BankAccountResourceIT {
  ...
  public void setup() {
    ...
    this.restBankAccountMockMvc =
      MockMvcBuilders.standaloneSetup(bankAccountResource)
  ...
  */
  ...
  public void createBankAccount() throws Exception {
    ...
    BankAccount testBankAccount = bankAccountList.get(
  ...
  public void checkNameIsRequired() throws Exception {
    ...
    restBankAccountMockMvc.perform(post("/api/bank-accounts").contentType(TestUtil.APPLICATION_JSON_UTF8).content(TestUtil.convertObjectToJsonBytes(bankAccount))).andExpect(status().isBadRequest());
  ...
  public void getBankAccount() throws Exception {
    ...
    // Get the bankAccount
    restBankAccountMockMvc.perform(get("/api/bank-accounts/{id}", bankAccount.getId())).andExpect(status().isOk()).andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE)).andExpect(jsonPath("$.id").value(bankAccount.getId().intValue())).andExpect(jsonPath("$.name").value(DEFAULT_NAME.toString())).andExpect(jsonPath("$.balance").value(DEFAULT_BALANCE.intValue()));
  }
  ...
  public void updateNonExistingBankAccount() throws Exception {
    ...
    restBankAccountMockMvc.perform(put("/api/bank-accounts").contentType(TestUtil.APPLICATION_JSON_UTF8).content(TestUtil.convertObjectToJsonBytes(bankAccount))).andExpect(status().isBadRequest());
  ...
  public void deleteBankAccount() throws Exception {
    ...
    restBankAccountMockMvc.perform(delete("/api/bank-accounts/{id}", bankAccount.getId()).accept(TestUtil.APPLICATION_JSON_UTF8)).andExpect(status().isNoContent());
