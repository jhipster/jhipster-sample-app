...
public class BankAccountGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all bankAccounts").get("/api/bank-accounts").headers(headers_http_authenticated).check(status().is(200)))
                ...
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created bankAccount").get("${new_bankAccount_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created bankAccount").delete("${new_bankAccount_url}").headers(headers_http_authenticated))
                .pause(10)
        );
